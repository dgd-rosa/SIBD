...
SELECT count(*), a.analyst_name, a.analyst_address
FROM Incident i
         JOIN analyses a on (i.instant = a.instant and i.element_id = a.element_id)
GROUP BY (a.analyst_name, a.analyst_address)
HAVING count(*) = (SELECT MAX(C)
                   FROM (SELECT count(*) as C
                         FROM Incident i
                                  JOIN analyses a on (i.instant = a.instant and i.element_id = a.element_id)
                         GROUP BY (a.analyst_name, a.analyst_address)) AS Q);

-- C. All substations with more than one transformer
SELECT s.gps_coords, s.locality_name, COUNT(*) as number_of_transformers
FROM transformer t
